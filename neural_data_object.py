...
class NeuralData():
    def __init__(self, mouse, date, base_name='/media/maclean/3d4f021d-2b12-4203-943d-4f1052e2ccf0/1104_hal_data/',
        ...
        self.old_red = False
        if os.path.exists(f'{self.proc_dir}red_labels.npy') or sure and os.path.exists(f'{self.proc_dir}red_labels.npy'):
            self.no_red = False
            if not sure:
        ...
        spks_file = self.s2p_dir + 'cascade_spks.npy'
        ...
        if use_l == 0:
            ...
            raise ValueError(f"Requested spike data (use_l={use_l}) does not exist in NeuralData. Check that 'cascade_spks.npy' exists.")
    ...
    def red_noise_correlations(self, vis_resp_only=False, easy=False, logs=False):
        if not hasattr(self, 'red_labels') or self.red_labels is None:
            raise ValueError("Red-labeled neurons not found.")
        
        red_idxs = self.red_labels
        resps, stimuli = self.get_norm_trial_responses()
        resps = [resp[red_idxs] for resp in resps]
        if logs:
            resps = [np.log(resp + 0.01) for resp in resps]
